...
draw.rect(screen,(30,30,30),(902,75,122,596))#rect for where all stickers are
...
pencilselect=transform.smoothscale(image.load("images/pencilselect.png"), (50,50))

eraserRect = Rect(3,115,56,56)
eraser=transform.smoothscale(image.load("images/eraser.png"), (46,46))
eraserselect=transform.smoothscale(image.load("images/eraserselect.png"), (46,46))

removeRect=Rect(5,14,30,30)
remove=transform.smoothscale(image.load("images/remove.png"),(1100//52,1303//52))
removeselect=transform.smoothscale(image.load("images/removeselect.png"),(1100//52,1303//52))

saveRect=Rect(5,728,35,35)
save=transform.smoothscale(image.load("images/save.png"),(28,28))
saveselect=transform.smoothscale(image.load("images/saveselect.png"),(28,28))

openRect=Rect(45,728,35,35)
openfile=transform.smoothscale(image.load("images/open.png"),(29,29))
openfileselect=transform.smoothscale(image.load("images/openselect.png"),(29,29))

paintRect=Rect(62,51,56,56)
paint=transform.smoothscale(image.load("images/paint.png"),(514//12,626//12))
paintselect=transform.smoothscale(image.load("images/paintselect.png"),(514//12,626//12))

pickerRect=Rect(62,115,56,56)
picker=transform.smoothscale(image.load("images/picker.png"),(54,54))
pickerselect=transform.smoothscale(image.load("images/pickerselect.png"),(54,54))
...
polygon=transform.smoothscale(image.load("images/polygon.png"),(600//13,600//13))
polyselect=transform.smoothscale(image.load("images/polyselect.png"),(600//13,600//13))
...
undoselect=transform.smoothscale(image.load("images/undoselect.png"),(26,26))

redoRect=Rect(75,14,30,30)
redo=transform.smoothscale(image.load("images/redo.png"),(26,26))
redoselect=transform.smoothscale(image.load("images/redoselect.png"),(26,26))
...
while running:
    ...
    for e in event.get():
        ...
        if e.type==MOUSEBUTTONUP:
            ...
            if e.button == 1:
                ...
                if canvasRect.collidepoint(mx,my) and tool!='picker' and tool!='fill' and tool!='polygon': #fill and polygon require adding to undo after their actions, picker doesn't draw
                    undos.append(screen.subsurface(canvasRect).copy()) #copies only whats on the canvas
                    #canCount = 1
    ...
    draw.rect(screen,(255,255,255),(95,530,25,25),2)#colour display outline

    screen.blit(remove,(9,16))#remove all icon
    draw.rect(screen,gray,removeRect,3)

    rgb = myfont.render(str(colour), False, (255, 255, 255))#displaying rgb values
    draw.rect(screen,(0),(0,535,95,25)) #adds black rect to allow rgb to change
    ...
    if tool!='eraser' and tool!='paint' and tool!='line' and tool!='unfrect' and tool!='unfcirc' and tool!='spray': #puts black areas over circles when other tools selected
    ...
    if removeRect.collidepoint(mx,my):
        ...
        if leftclick:
            draw.rect(screen,(255,255,255),canvasRect) #makes canvas white (removes all)
    ...
    if openRect.collidepoint(mx,my):
        ...
        if leftclick:
            ...
            if result:
                screen.blit(image.load(result),(122,51)) #adds image that was loaded
    ...
    if undoRect.collidepoint(mx,my):
        ...
        if leftclick:
            if len(undos)>=2: #the undos must be at or over 2 because it already starts at 1
                if len(polypoints)>0:
                    polyredos.append(polypoints[-1])#this is for continuing polygon when undoing
                    del polypoints[-1]
                screen.blit(undos[-2],(122,51)) #second last undo is blitted
                redos.append(undos[-1]) #need to add last undo to redo for reuse
    ...
    if redoRect.collidepoint(mx,my):
        ...
        if leftclick:
            if len(redos)>=1: #starts at 0, so should be at or over 1
                if len(polypoints)>0:
                    polypoints.append(polyredos[-1]) #polygon can then be continued
                    del polyredos[-1]
    ...
    if len(redos)>=1: #this deletes all redos if the list has anything in it and you click
        if canvasRect.collidepoint(mx,my):
            if leftclick and tool!='picker':
                del redos[:]
            if rightclick and tool=='polygon' and len(polypoints>=3): #can only delete all redos if the polygon is closed by rightclicking
                del redos[:]
...
#-----------------------------------DRAWING--------------------------------
    if canvasRect.collidepoint(mx,my):
        ...
        if mb[0]==1:
            canCount = 1
            if tool=='picker':
                colour=screen.get_at((mx,my)) #rgba values of mouse location
            ...
            if tool=='eraser':
                if lastpos is not None:
                    dist=sqrt((my-lastpos[1])**2+(mx-lastpos[0])**2) #distance between last position and current
                    ...
                    for i in range(int(dist)):
                        ...
                        draw.circle(screen,white,(x, y),size) #creates circles between each position where they're missing
                    draw.circle(screen,white,(mx, my),size) #initial circles
                lastpos=mx,my
            ...
            if tool=='sans':
                ...
                screen.blit(sans,(mx-sans.get_size()[0]//2,my-sans.get_size()[1]//2)) #sticker in center of cursor

            if tool=='undyne':
                screen.blit(back,(0,0))
                screen.blit(undyne,(mx-undyne.get_size()[0]//2,my-undyne.get_size()[1]//2))

            if tool=='papyrus':
                screen.blit(back,(0,0))
                screen.blit(papyrus,(mx-papyrus.get_size()[0]//2,my-papyrus.get_size()[1]//2))

            if tool=='frisk':
                screen.blit(back,(0,0))
                screen.blit(frisk,(mx-frisk.get_size()[0]//2,my-frisk.get_size()[1]//2))

            if tool=='flowey':
                screen.blit(back,(0,0))
                screen.blit(flowey,(mx-flowey.get_size()[0]//2,my-flowey.get_size()[1]//2))
            ...
            if tool=='line': #same as eraser, though starting position (when first clicking) is instead used and screen is copied
                screen.blit(back,(0,0))
            ...
            if tool=='unfrect':
                screen.blit(back,(0,0))
                draw.polygon(screen,colour,[(start),(mx,start[1]),(mx,my),(start[0],my)],size) #draw polygon is better because each point is specified

            if tool=='frect':
                screen.blit(back,(0,0))
                draw.polygon(screen,colour,[(start),(mx,start[1]),(mx,my),(start[0],my)])

            if tool=='fcirc':
                screen.blit(back,(0,0))
                ellipseRect=Rect(start[0],start[1],mx-start[0],my-start[1]) #ellipse at start with width and height distance between mouse and start
                ellipseRect.normalize() #corrects negative values
                draw.ellipse(screen,colour,ellipseRect)

            if tool=='unfcirc':
                screen.blit(back,(0,0))
                ellipseRect=Rect(start[0],start[1],mx-start[0],my-start[1])
            ...
            if tool=='spray':
                if lastpos is not None:
                    ...
                    for i in range(int(dist)):
                        x = int(lastpos[0]+i/dist*distx)
                        y = int(lastpos[1]+i/dist*disty)
                        orx,ory=randint(x-size,x+size),randint(y-size,y+size)
                        screen.set_at((orx,ory),colour)
                    randx,randy=randint(mx-size,mx+size),randint(my-size,my+size)
        ...
        if tool=='polygon':
            if leftclick:
                ...
                if len(polypoints)>1:
                    draw.line(screen,colour,(polypoints[len(polypoints)-2]),(mx,my))
                    undos.append(screen.subsurface(canvasRect).copy())
            if rightclick:
                if len(polypoints)>=3:
